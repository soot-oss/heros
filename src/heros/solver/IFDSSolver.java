...
/**
 * A solver for an {@link IFDSTabulationProblem}. This solver in effect uses the
 * {@link IDESolver} to solve the problem, as any IFDS problem can be intepreted
 * as a special case of an IDE problem. See Section 5.4.1 of the SRH96 paper. In
 * effect, the IFDS problem is solved by solving an IDE problem in which the
 * environments (D to N mappings) represent the set's characteristic function.
 * 
 * @param <N>
...
public class IFDSSolver<N, D, M, I extends InterproceduralCFG<N, M>>
		extends IDESolver<N, D, M, IFDSSolver.BinaryDomain, I> {
	...
	/**
	 * Creates a solver for the given problem. The solver must then be started
	 * by calling {@link #solve()}.
	 */
	...
	static <N, D, M, I extends InterproceduralCFG<N, M>> IDETabulationProblem<N, D, M, BinaryDomain, I> createIDETabulationProblem(
		...
		return new IDETabulationProblem<N, D, M, BinaryDomain, I>() {
			...
			class IFDSEdgeFunctions implements EdgeFunctions<N, D, M, BinaryDomain> {
				...
				public EdgeFunction<BinaryDomain> getCallEdgeFunction(D d1, N callStmt, D srcNode, M destinationMethod,
						D destNode) {
					if (srcNode == ifdsProblem.zeroValue())
						return ALL_BOTTOM;
					return EdgeIdentity.v();
				...
				public EdgeFunction<BinaryDomain> getReturnEdgeFunction(D d1, N callSite, M calleeMethod, N exitStmt,
						D exitNode, N returnSite, D retNode) {
					if (exitNode == ifdsProblem.zeroValue())
						return ALL_BOTTOM;
					return EdgeIdentity.v();
				}

				public EdgeFunction<BinaryDomain> getCallToReturnEdgeFunction(D d1, N callStmt, D callNode,
